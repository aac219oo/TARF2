<template>
  ...
  <el-container>
    ...
    <el-main>
        ...
        <template class="maintenance_add_wrap">
          ...
          <el-dialog v-model="dialogFormVisible" center align-center :close-on-click-modal="false" @close="cancel"
            ...
            <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules">
              <el-form-item label="帳號" :label-width="formLabelWidth" prop="EmplSeri">
                <el-input v-model="ruleForm.EmplSeri" @keyup.enter="submitForm(ruleFormRef)" placeholder="請輸入"
                  clearable />
              </el-form-item>
              ...
              <el-form-item label="公司名稱" :label-width="formLabelWidth" prop="DeptName">
                <el-input v-model="ruleForm.DeptName" @keyup.enter="submitForm(ruleFormRef)" placeholder="請輸入"
                  clearable />
              </el-form-item>
              <el-form-item label="權限種類" :label-width="formLabelWidth" prop="RighItem">
                <el-select v-model="ruleForm.RighItem" filterable default-first-option :reserve-keyword="false"
                  placeholder="請選擇">
                  <el-option v-for="item in optionsRighItem" :key="item.value" :label="item.label"
                    :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="統一編號" :label-width="formLabelWidth" prop="CustUnifNo">
                <el-input v-model="ruleForm.CustUnifNo" @keyup.enter="submitForm(ruleFormRef)" placeholder="請輸入"
                  clearable />
              </el-form-item>
              <el-form-item label="標號" :label-width="formLabelWidth" prop="ProjIdStr">
                <el-select v-model="ruleForm.ProjIdStr" multiple filterable default-first-option
                  :reserve-keyword="false" placeholder="請選擇">
                  <el-option v-for="(item, index) in optionsProjIdStr" :key="index" :label="item.proJ_ID"
                    :value="item.proJ_ID" />
                </el-select>
              </el-form-item>
            </el-form>
      ...
      <el-table :data="tableData" class="tableComUsing" v-loading="loading" border stripe style="width: 100%"
        :header-cell-style="{
          background: '#ebf4f9',
          color: '#000',
          textAlign: 'center',
        }">
        <el-table-column type="index" label="序號" width="70" :resizable="false" />
        <el-table-column label="帳號" prop="useR_ID" sortable width="160px" :resizable="false" />
        <el-table-column label="姓名" prop="useR_NAME" width="130px" sortable :resizable="false" />
        <el-table-column label="公司名稱" prop="depT_NAME" sortable :resizable="false" />
        <el-table-column label="權限狀態" prop="rigH_STAT_DESC" sortable width="130px" :resizable="false" />
        <el-table-column label="權限種類" prop="rigH_ITEM_DESC" sortable width="130px" :resizable="false" />
        <el-table-column label="註銷" width="89" :resizable="false">
          <template #default="scope">
            <el-switch v-model="scope.row.revokE_FLAG" :active-value="1" :inactive-value="0"
              @change="changeStatus($event, scope.row, scope.$index)" />
          </template>
        </el-table-column>
        <el-table-column label="標號維護" width="89" :resizable="false">
          <template #default="scope">
            <el-button link @click="openQueryProjData(scope.row)" class="button_edit">
              <img src="../../assets/icon04.png" style="width: 24px; vertical-align: bottom" alt="" />
            </el-button>
            <el-dialog v-model="scope.row.openQueryProjData" :append-to-body="true" center align-center
              :close-on-click-modal="false" title="標號維護">
              <el-form :model="QueryProjData">
                <el-form-item label="標號" :label-width="formLabelWidth">
                  <el-select v-model="QueryProjData" multiple filterable default-first-option :reserve-keyword="false">
                    <!-- @change="$forceUpdate()" -->
                    <el-option v-for="(item, index) in optionsProjIdStr" :key="index" :label="item.proJ_ID"
                      :value="item.proJ_ID" />
                  </el-select>
              ...
              <template #footer>
                <span class="dialog-footer">
                  <el-button @click="cancelQueryProjData(scope.row)">取消</el-button>
                  <el-button type="primary" @click="submitProjData(scope.row, QueryProjData)">
                    儲存
                  </el-button>
                </span>
              </template>
...
const submitProjData = (row, val) => {
  // row.openQueryProjData = false
  console.log(val)
  const urlSaveProj = url + "SaveProj?EmplSeri=" + row["Empl_Seri"] + "&UserId=" + row["useR_ID"] + "&ProjIdStr=" + val.join(";")
