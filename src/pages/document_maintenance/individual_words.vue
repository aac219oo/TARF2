<template>
	<el-container>
		...
		<el-main>
			<el-menu :default-active="activeIndex" class="el-menu-demo index_row" mode="horizontal"
				@select="handleSelect">
			...
			<el-table :data="tableData" v-loading="loading" border stripe style="width: 100%; max-width: 800px"
				...
				<el-table-column :prop="item.prop" :label="item.label" v-for="(item, index) in tableHeader"
					:key="item.prop" :resizable="false">
				...
				<el-table-column width="120px" align="right" :resizable="false">
					...
					<template #default="scope">
						<el-popconfirm width="170px" title="確定要刪除嗎?" confirm-button-text="是" cancel-button-text="否"
							...
							<template #reference>
								<el-button>
									<img src="../../assets/icon06.png" style="width: 24px; vertical-align: bottom"
										alt="" />
								</el-button>
							</template>
						</el-popconfirm>
						<el-button link size="small" v-if="!scope.row.editable" @click="handleEdit(scope.row)"
							class="button_edit">
							<img src="../../assets/icon04.png" style="width: 24px; vertical-align: bottom" alt="" />
						</el-button>
						<el-button link size="small" v-if="scope.row.editable" @click="handleSave(scope.row)"
							class="button_edit">
...
const handleAdd = (row) => {
	...
	if (AddorEdit.value) { //執行新增
		...
		const urlAdd = url + "SavingNew?UserId=" + UserId + "&argPhraseDesc=" + row["phrasE_DESC"]; //取得新增資料的API
		axios
			.get(urlAdd)
			.then((res) => {
		...
		const UserId = sessionStorage.getItem("UserId");
		const urlEdit = url + "SavingModify?UserId=" + UserId + "&argPhraseDesc=" + row["phrasE_DESC"] + "&argPhraseDescDB=" + argPhraseDescDB.value; //取得編輯資料的API，並回傳舊資料的值
		axios
			.get(urlEdit)
			.then((res) => {
				const statusCode = res.data[0].statusCode; //儲存狀態代碼
...
const deleteRow = (index: number) => {
	// 儲存session
	const UserId = sessionStorage.getItem("UserId");
	const urlDelete = url + "DeletePhraseBasc?UserId=" + UserId + "&argPhraseDesc=" + index["phrasE_DESC"];  // 儲存刪除api
	axios
		.get(urlDelete)
		.then((res) => {
