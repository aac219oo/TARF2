    ...
    <el-main class="contact_main">
      ...
      <el-row>
        ...
        <el-table class="tableForm tableContactCom" v-loading="loading" :data="filterTableData" stripe border
          ...
          <el-table-column prop="proJ_ID" :sortable="sortable" :resizable="false"><template #header>
              <p>標號</p>
              <el-input v-model="searchProJ_ID" class="w-50 m-2" size="default" placeholder="搜尋" :suffix-icon="Search"
                @keypress="stopEnter" @click.stop.prevent="stopSortable" />
            </template></el-table-column>
          <el-table-column prop="depT_NAME" :sortable="sortable" :resizable="false"><template #header>
              <p>單位</p>
              <el-input v-model="searchDepT_NAME" class="w-50 m-2" size="default" placeholder="搜尋" :suffix-icon="Search"
                @keypress="stopEnter" @click.stop.prevent="stopSortable" />
            </template></el-table-column>
          <el-table-column prop="useR_NAME" :sortable="sortable" :resizable="false"><template #header>
              <p>姓名</p>
              <el-input v-model="searchUseR_NAME" class="w-50 m-2" size="default" placeholder="搜尋" :suffix-icon="Search"
                @keypress="stopEnter" @click.stop.prevent="stopSortable" />
            </template></el-table-column>
